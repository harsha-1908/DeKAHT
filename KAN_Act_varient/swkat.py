...
# ============================================================
# KAN ACTIVATION (NEW — FULL KAN NONLINEARITY)
# ============================================================
class KANActivation1D(nn.Module):
    """
    Channel-wise learnable KAN activation using linear spline interpolation.
    Replaces fixed GELU entirely.
    """
    def __init__(self, channels: int, grid_size: int = 16,
        ...
        # Learnable control values per channel
        self.coeffs = nn.Parameter(torch.zeros(channels, grid_size))
        nn.init.normal_(self.coeffs, mean=0.0, std=0.02)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_clamped = torch.clamp(x_flat, self.x_min, self.x_max)
        t = (x_clamped - self.x_min) / (self.x_max - self.x_min) * (self.grid_size - 1)

        idx0 = torch.floor(t).long().clamp(0, self.grid_size - 2)
        idx1 = idx0 + 1
        w = t - idx0.float()

        c0 = torch.gather(self.coeffs, 1, idx0)
        c1 = torch.gather(self.coeffs, 1, idx1)

        y = (1.0 - w) * c0 + w * c1
        y = y.reshape(C, B, H, W).permute(1, 0, 2, 3)
        return y


# ---------------------------
# Pure-PyTorch GR_KAN_Conv (FULL KAN FFN)
# ---------------------------
class GR_KAN_Conv(nn.Module):
    """
    Pure PyTorch KAN-style FFN used as an MLP replacement.
    FULL KAN: fixed GELU is replaced by learnable activation.
    """
    def __init__(self,
                 channels: int,
        ...
        assert hidden_channels % n_groups == 0
        self.group_scale = nn.Parameter(torch.ones(n_groups, hidden_channels // n_groups))

        # ?? FULL KAN ACTIVATION (replaces GELU)
        self.act = KANActivation1D(hidden_channels)
