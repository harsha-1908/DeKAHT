...
from dekaht import SwkatGRKAN_Opt, set_prefer_kat, GR_KAN_Conv, SwkatBlockOpt
...
def compute_kan_reg(model):
    """
    Lightweight L2 regularization for KAN parameters only.
    Prevents late-epoch activation collapse.
    """
    reg = 0.0
    for name, param in model.named_parameters():
        ...
        if (
            "group_scale" in name
            or "gate" in name
            or "kan" in name.lower()
        ):
            reg = reg + (param ** 2).mean()
    return reg
    ...
    for i, (x, y) in enumerate(loader):
        ...
        if cut_alpha and cut_alpha > 0:
            ...
            with autocast(enabled=amp_enabled):
                ...
                loss = loss + lambda_kan * compute_kan_reg(model)
...
def enforce_kan_only(model, is_main=True):
    """
    Verifies that every SwkatBlockOpt uses GR_KAN_Conv as its mlp_conv.
    Raises RuntimeError with diagnostics if not.
    """
    problems = []
        ...
        if isinstance(m, SwkatBlockOpt):
            ...
            if mlp is None:
                ...
                if hasattr(mlp, "is_kan") and getattr(mlp, "is_kan") is True:
                    continue
                if not isinstance(mlp, GR_KAN_Conv):
                    problems.append(f"{name}: mlp_conv is {type(mlp).__name__} (expected GR_KAN_Conv)")
    if problems:
        msg = "KAN-only enforcement failed: the following SwkatBlockOpt instances do not use GR_KAN_Conv:\n"
        msg += "\n".join(problems)
        if is_main:
            print(msg)
        ...
        if is_main:
            print("[KAN-ENFORCE] All SwkatBlockOpt blocks verified to use GR_KAN_Conv (100% KAN).")
...
def main():
    ...
    if kat_requested:
        try:
            ...
            # Do not allow fall back to a 'vanilla MLP' — but compiled kat is optional for perf only.
    ...
    if args.use_custom_kan:
        enforce_kan_only(model, is_main=is_main)
    else:
        if is_main:
            print("[KAN-ENFORCE] Skipped enforcement (use --use-custom-kan to enable strict GR_KAN_Conv checks).")
