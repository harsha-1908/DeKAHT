...
# Full training script for SW-KAT hybrid model (non-shifted windows -> KAN-FFN, shifted windows -> Std MLP-FFN)
# Preserves original training features: EMA, SAM, SWA, resolution ramping, mixup/cutmix, AMP handling, metrics, attention maps.
...
# import hybrid model entrypoint and types (make sure swkat_hybrid.py is present)
from dekaht_hybrid import SwkatGRKAN_Opt, set_prefer_kat, GR_KAN_Conv, SwkatBlockOpt, enforce_kan_only
...
def parse_args():
    ...
    # <-- ADDED: single "checkbox" to require the custom pure-PyTorch KAN (GR_KAN_Conv) enforcement for hybrid rule
    p.add_argument('--use-custom-kan', action='store_true', help='Require hybrid rule: non-shifted blocks must use GR_KAN_Conv and shifted blocks use Std_MLP_Conv. If set, script will fail early if violation detected.')
    # gradient clipping
...
def main():
    ...
    # ENFORCE hybrid rule if user requested it (fail early)
    if args.use_custom_kan:
        enforce_kan_only(model, is_main=is_main)
    else:
        if is_main:
            print("[KAN-HYBRID-ENFORCE] Skipped enforcement (use --use-custom-kan to enable hybrid verification).")
