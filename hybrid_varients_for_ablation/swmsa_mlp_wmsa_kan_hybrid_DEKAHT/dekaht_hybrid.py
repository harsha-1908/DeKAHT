#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# swkat_hybrid.py - Hybrid SW-KAT: non-shifted windows -> KAN-FFN, shifted-windows -> MLP-FFN
# - Keeps pure-PyTorch GR_KAN_Conv for KAN FFN path
# - Adds Std_MLP_Conv for shifted (sw-MSA) blocks
# - Minimal API changes to the original swkat.py; drop-in replacement for model usage
...
# ---------------------------
# Std MLP conv-based FFN for shifted-window blocks (no spatial depthwise mixing)
# ---------------------------
class Std_MLP_Conv(nn.Module):
    """
    Standard conv-MLP style FFN for shifted-window blocks.
    Simpler than KAN: pointwise expand -> activation -> pointwise project.
    Accepts (B, C, H, W) and returns same.
    """
    def __init__(self, channels: int, hidden_channels: Optional[int] = None, drop: float = 0.0):
        ...
        self.fc2 = nn.Conv2d(hidden_channels, channels, kernel_size=1, bias=True)
        self.drop = nn.Dropout2d(drop) if drop > 0.0 else nn.Identity()
        self.is_kan = False  # mark that this is not a KAN block
...
class WindowAttention(nn.Module):
    def __init__(self, dim: int, window_size: int, num_heads: int, qkv_bias: bool = True,
        ...
        self.relative_position_bias_table = nn.Parameter(torch.zeros(num_relative_positions, self.num_heads))
...
# ---------------------------
# SwkatBlockOpt-like Block (now picks FFN based on shift_size)
# ---------------------------
class SwkatBlockOpt(nn.Module):
    def __init__(self, dim: int, input_resolution: Tuple[int, int], num_heads: int, window_size: int = 7,
        ...
        # SELECT FFN BASED ON SHIFT: non-shifted (w-MSA) -> KAN (GR_KAN_Conv)
        # shifted (sw-MSA) -> standard mlp conv (Std_MLP_Conv)
        if self.shift_size == 0:
            self.mlp_conv = GR_KAN_Conv(channels=dim, hidden_channels=int(dim * mlp_ratio), groups=groups, drop=0.0)
        else:
            # shifted windows get the standard MLP-like conv (no depthwise local mixing)
            self.mlp_conv = Std_MLP_Conv(channels=dim, hidden_channels=int(dim * mlp_ratio), drop=0.0)

        self.attn_mask = None
        self._mask_built_for = None
...
# ---------------------------
# PatchEmbed and PatchMerging (unchanged)
# ---------------------------
...
# ---------------------------
# Enforcement helper: hybrid-aware
# ---------------------------
def enforce_kan_only(model, is_main=True):
    """
    Verifies hybrid rule:
      - non-shifted (w-MSA, shift_size==0) SwkatBlockOpt must use GR_KAN_Conv
      - shifted (sw-MSA, shift_size>0) SwkatBlockOpt must NOT be GR_KAN_Conv (expects Std_MLP_Conv)
            ...
            if mlp is None:
                ...
                if m.shift_size == 0:
                    ...
                    if hasattr(mlp, "is_kan") and getattr(mlp, "is_kan") is True:
                        ...
                        problems.append(f"{name}: expected GR_KAN_Conv for non-shifted block but got {type(mlp).__name__}")
                else:
                    # expect standard MLP (non-KAN)
                    if hasattr(mlp, "is_kan") and getattr(mlp, "is_kan") is False:
                        continue
                    else:
                        problems.append(f"{name}: expected Std_MLP_Conv for shifted block but got {type(mlp).__name__}")
    if problems:
        msg = "KAN-only/hybrid enforcement failed: the following SwkatBlockOpt instances violate the hybrid rule:\n"
        msg += "\n".join(problems)
        if is_main:
            print(msg)
        ...
        if is_main:
            print("[KAN-HYBRID-ENFORCE] Hybrid rule verified: non-shifted(w-MSA)=KAN, shifted(sw-MSA)=MLP.")

# module exports
__all__ = ["SwkatGRKAN_Opt", "STANDARD_VARIANTS", "set_prefer_kat", "GR_KAN_Conv", "Std_MLP_Conv", "WindowAttention", "SwkatBlockOpt", "enforce_kan_only"]
